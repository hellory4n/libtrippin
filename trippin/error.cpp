...
tr::FileError tr::FileError::from_win32(tr::String patha, tr::String pathb, tr::FileOperation operation)
{
	...
	switch (man.win32_code) {
		case ERROR_FILE_NOT_FOUND: t = FileErrorType::NOT_FOUND; break;
		case ERROR_ACCESS_DENIED: t = FileErrorType::ACCESS_DENIED; break;
		case ERROR_SHARING_VIOLATION: t = FileErrorType::DEVICE_OR_RESOURCE_BUSY; break;
		case ERROR_BUSY: t = FileErrorType::DEVICE_OR_RESOURCE_BUSY; break;
		case ERROR_DISK_FULL: t = FileErrorType::NO_SPACE_LEFT; break;
		case ERROR_FILE_EXISTS: t = FileErrorType::FILE_EXISTS; break;
		case ERROR_ALREADY_EXISTS: t = FileErrorType::FILE_EXISTS; break;
		case ERROR_INVALID_HANDLE: t = FileErrorType::BAD_HANDLE; break;
		case ERROR_GEN_FAILURE: t = FileErrorType::HARDWARE_ERROR_OR_UNKNOWN; break;
		case ERROR_IO_DEVICE: t = FileErrorType::HARDWARE_ERROR_OR_UNKNOWN; break;
		case ERROR_CANNOT_MAKE: t = FileErrorType::IS_DIRECTORY; break;
		// TODO its not really that but i dont care
		case ERROR_PATH_NOT_FOUND: t = FileErrorType::IS_NOT_DIRECTORY; break;
		case ERROR_TOO_MANY_OPEN_FILES: t = FileErrorType::TOO_MANY_OPEN_FILES; break;
		case ERROR_BROKEN_PIPE: t = FileErrorType::BROKEN_PIPE; break;
		case ERROR_FILENAME_EXCED_RANGE: t = FileErrorType::FILENAME_TOO_LONG; break;
		case ERROR_INVALID_PARAMETER: t = FileErrorType::INVALID_ARGUMENT; break;
		case ERROR_WRITE_PROTECT: t = FileErrorType::READ_ONLY_FILESYSTEM; break;
		case ERROR_NEGATIVE_SEEK: t = FileErrorType::ILLEGAL_SEEK; break;
		case ERROR_DIR_NOT_EMPTY: t = FileErrorType::DIRECTORY_NOT_EMPTY; break;
	}
...
tr::String tr::FileError::message()
{
	...
	if (this->errno_code != 0) {
		...
		if (this->op == FileOperation::MOVE_FILE) {
			return tr::sprintf(tr::scratchpad(), "%s (source '%s', destination '%s', win32 error %i): %s",
				operation.buf(), this->path_a.buf(), this->path_b.buf(), this->win32_code, error.buf()
			);
		}
		else {
			return tr::sprintf(tr::scratchpad(), "%s (path '%s', win32 error %i): %s",
				operation.buf(), this->path_a.buf(), this->win32_code, error.buf()
			);
		}
	}
	#endif

...
struct Vec3
{
	...
	constexpr Vec3& operator*=(T other)
	{
		return *this = *this * other;
	}
	constexpr Vec3& operator/=(T other)
	...
	// warning: implicit conversion loses floating-point precision: 'float64' (aka 'double') to
	// 'float'
...
struct Vec4
{
	...
	constexpr Vec4& operator*=(T other)
	{
		return *this = *this * other;
	}
	constexpr Vec4& operator/=(T other)
...
constexpr Vec2<T> lerp(Vec2<T> a, Vec2<T> b, float64 t)
{
	auto man = Vec2<float64>(1.0 - t, 1.0 - t) * Vec2<float64>(a.x, a.y) +
		   Vec2<float64>(t, t) * Vec2<float64>(b.x, b.y);
...
Vec3<T> lerp(Vec3<T> a, Vec3<T> b, float64 t)
{
	auto man = Vec3<float64>(1.0 - t, 1.0 - t, 1.0 - t) * Vec3<float64>(a.x, a.y, a.z) +
		   Vec3<float64>(t, t, t) * Vec3<float64>(b.x, b.y, b.z);
...
struct Color
{
	...
	{
		return {static_cast<uint8>(clamp(r + c.r, 0, 255)),
			static_cast<uint8>(clamp(g + c.g, 0, 255)),
			static_cast<uint8>(clamp(b + c.b, 0, 255)),
			static_cast<uint8>(clamp(a + c.a, 0, 255))};
	}
	constexpr Color operator-(Color c) const
	{
		return {static_cast<uint8>(clamp(r - c.r, 0, 255)),
	...
	constexpr Color operator*(uint8 c) const
	{
		return {static_cast<uint8>(clamp(r * c, 0, 255)),
			static_cast<uint8>(clamp(g * c, 0, 255)),
			static_cast<uint8>(clamp(b * c, 0, 255)),
			static_cast<uint8>(clamp(a * c, 0, 255))};
	}
	constexpr Color operator/(Color c) const
	{
		return {static_cast<uint8>(clamp(r / c.r, 0, 255)),
	...
	constexpr Color& operator*=(uint8 other)
	{
		return *this = *this * other;
	}
	constexpr Color& operator/=(uint8 other)
...
struct Matrix4x4
{
	...
	// Note this initializes the matrix as 0, not identity (use `Matrix4x4::identity()` for
	// that)
	constexpr Matrix4x4()
	{
		for (usize i = 0; i < 4; i++) {
